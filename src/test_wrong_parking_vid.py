...
VIDEO_PATH = os.path.join(BASE_DIR, "src", "wrongparkingtest.mp4")
OUTPUT_VIDEO_PATH = os.path.join(BASE_DIR, "parking_output_wrong.mp4")
...
# -------- Wrong parking (STRICTER NOW) --------
WRONG_PARKING_IOU_THRESHOLD = 0.15
...
def is_car_in_slot(slot_box, car_box, iou_threshold=0.15):
    ...
    1) the car bottom-center is inside the slot
    OR
    2) IoU overlap is enough
    """
...
def detect_wrong_parking(cars, frozen_slots, min_overlap=0.15, similarity_ratio=0.75):
    """
    Wrong parking if:
    - the car overlaps at least 2 slots
    - and the top 2 overlaps are both meaningful
    - and the second overlap is close enough to the first
      (meaning the car is shared between two slots)
    """
    wrong_events = []
    ...
    for car_idx, car in enumerate(cars):
        ...
        for slot in frozen_slots:
            overlap = iou(car_box, slot["bbox"])
            if overlap >= min_overlap:
                overlaps.append((slot["slot_id"], overlap))
        ...
        if len(overlaps) >= 2:
            ...
            # if second overlap is close to first overlap,
            # car is probably between two slots
            if ov2 >= ov1 * similarity_ratio:
                wrong_events.append({
                    "car_index": car_idx,
...
def main():
    ...
    similarity_ratio=0.75
        ...
        # =================================
        # SLOT OCCUPANCY + SMOOTHING + TIMER
        # =================================
        occupied_count = 0
        empty_count = 0
